...
- Build **signed graphs** via analytical p-values and **FDR correction**.  
- Perform **community detection** minimizing BIC (signed SBM) or frustration.  
...
T.analytical_signature_dist # analytical distribution objects / PMFs
```
...
From the empirical signature and the fitted model, you can construct a signed adjacency matrix using analytical p-values and FDR correction:
...
- For `model='bSRGM'` and `model='bSCM'`, SIBERIA:
  ...
  - Applies Benjamini–Hochberg FDR correction on the upper triangle.  
